...
function findFirstSingleChar(str) {
  ...
  strTemp.forEach((item, index) => {
    if (res === null && !strTemp.slice(0, index).concat(strTemp.slice(index + 1)).includes(item)) {
      res = strTemp[index];
    }
  });
  return res;
...
function isCreditCardNumber(ccn) {
  const ccnDigits = ccn.toString().split('').map((item) => +item);
  const controlDigit = ccnDigits.pop();
  const sumDigit = ccnDigits.reverse().reduce((a, item, ind) => {
    let i = 0;
    if (!(ind % 2)) i = (2 * item).toString().split('').map((it) => +it).reduce((s, e) => s + e);
    else i = item;
    return a + i;
  }, 0);
  const calcControl = (10 - (sumDigit % 10)) % 10;
  return calcControl === controlDigit;
}
...
function isBracketsBalanced(str) {
  ...
  str.split('').forEach((char) => {
    if (brackets.open.includes(char)) stack.push(char);
    else if (stack.length) {
      if (brackets.close.indexOf(char) === brackets.open.indexOf(stack.at(-1))) {
        stack.pop();
      } else {
        stack.push('!');
      }
...
function getCommonDirectoryPath(pathes) {
  ...
  const commonDir = [];
  let tempArr = [];
  const minLength = Math.min(...pathesArr.map((item) => item.length));
  for (let j = 0; j < minLength; j += 1) {
    tempArr = pathesArr.map((item) => item[j]).sort();
    if (tempArr[0] === tempArr.at(-1)) commonDir.push(tempArr[0]);
    else break;
  }
  return commonDir.concat('').join('/');
}
